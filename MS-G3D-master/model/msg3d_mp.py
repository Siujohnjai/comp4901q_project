...
class Model(nn.Module):
    def __init__(self,
        ...
        # r=3 STGC blocks
        self.gcn3d1 = MultiWindow_MS_G3D(3, c1, A_binary, num_g3d_scales, window_stride=1).to(0)
        self.sgcn1 = nn.Sequential(
            MS_GCN(num_gcn_scales, 3, c1, A_binary, disentangled_agg=True),
            MS_TCN(c1, c1),
            MS_TCN(c1, c1)).to(0)
        self.sgcn1[-1].act = nn.Identity().to(0)
        self.tcn1 = MS_TCN(c1, c1).to(0)

        self.gcn3d2 = MultiWindow_MS_G3D(c1, c2, A_binary, num_g3d_scales, window_stride=2).to(0)
        self.sgcn2 = nn.Sequential(
            MS_GCN(num_gcn_scales, c1, c1, A_binary, disentangled_agg=True),
            MS_TCN(c1, c2, stride=2),
            MS_TCN(c2, c2)).to(0)
        self.sgcn2[-1].act = nn.Identity().to(0)
        self.tcn2 = MS_TCN(c2, c2).to(0)

        self.gcn3d3 = MultiWindow_MS_G3D(c2, c3, A_binary, num_g3d_scales, window_stride=2).to(1)
        self.sgcn3 = nn.Sequential(
            MS_GCN(num_gcn_scales, c2, c2, A_binary, disentangled_agg=True),
            MS_TCN(c2, c3, stride=2),
            MS_TCN(c3, c3)).to(1)
        self.sgcn3[-1].act = nn.Identity().to(1)
        self.tcn3 = MS_TCN(c3, c3).to(1)

        self.fc = nn.Linear(c3, num_class).to(1)

    def forward(self, x):
        N, C, T, V, M = x.size()
        x = x.permute(0, 4, 3, 1, 2).contiguous().view(N, M * V * C, T)
        x = self.data_bn(x)
        x = x.view(N * M, V, C, T).permute(0,2,3,1).contiguous().to(0)

        splits = iter(x.split(8, dim=0))
        s_next = next(splits)
        # Apply activation to the sum of the pathways
        s_prev = F.relu(self.sgcn1(s_next) + self.gcn3d1(s_next), inplace=True)
        s_prev = self.tcn1(s_prev)

        s_prev = F.relu(self.sgcn2(s_prev) + self.gcn3d2(s_prev), inplace=True)
        s_prev = self.tcn2(s_prev)
        s_prev = s_prev.to(1)

        ret = []
        for s_next in splits:
            s_prev = F.relu(self.sgcn3(s_prev) + self.gcn3d3(s_prev), inplace=True)
            s_prev = self.tcn3(s_prev)
