...
class Feeder(Dataset):
    ...
    def top_k(self, score, top_k):
        ...
        left = len(self.label) // world_size * (global_rank) 
        right= len(self.label) // world_size * (global_rank+1) 
        print(left, " ", right)
        if (global_rank != world_size-1):
            hit_top_k = [l in rank[i, -top_k:] for i, l in enumerate(self.label[left:right])]
        else:
            hit_top_k = [l in rank[i, -top_k:] for i, l in enumerate(self.label[left:])]
        
        print(sum(hit_top_k) * 1.0 / len(hit_top_k))
