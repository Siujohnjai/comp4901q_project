...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help='Distributed mode; default false')
    # parser.add_argument(
    #     '--distributed_args',
    #     type=dict,
    #     default=dict(),
    #     help='the arguments of distributed data parallel')
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if self.arg.distributed:
            ...
            self.arg.gpu = self.arg.local_rank
            torch.cuda.set_device(self.arg.gpu)
            torch.distributed.init_process_group(backend='nccl',
                                                init_method='env://')
            self.arg.world_size = torch.distributed.get_world_size()
            self.arg.rank = torch.distributed.get_rank()

        self.ngpus_per_node = torch.cuda.device_count()
        # print(self.ngpus_per_node)
        self.arg.batch_size = int(self.arg.batch_size / self.arg.world_size)
        self.arg.test_batch_size = int(self.arg.test_batch_size/ self.arg.world_size)
        ...
        if self.arg.distributed:
            # if len(self.arg.device) > 1
            # self.print_log(f'{len(self.arg.device)} GPUs, {n_node} available, using DataParallel')
            self.model = DDP(
                self.model
            )
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            ## Distributed
            self.sampler['train'] = torch.utils.data.distributed.DistributedSampler(
            ...
            # print(list(self.data_loader['train']))

        test_dataset = Feeder(**self.arg.test_feeder_args)
        self.sampler['test'] = torch.utils.data.distributed.DistributedSampler(
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            real_batch_size = int(self.arg.forward_batch_size / self.arg.world_size)
            ...
            for i in range(splits):
                ...
                loss_values.append(reduced_loss.item())
                
                if self.arg.rank == 0:            
                    # Display loss
                    process.set_description(f'(BS {real_batch_size}) loss: {reduced_loss.item():.4f} acc: {acc.item()*100:.2f}%')

                    self.train_writer.add_scalar('acc', acc, self.global_step)
                    self.train_writer.add_scalar('loss', reduced_loss.item() * splits, self.global_step)
        ...
        if save_model and self.arg.rank == 0:
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                for batch_idx, (data, label, index) in enumerate(process):
                    ...
                    accuracy = accuracy_topk(output, label, self.arg.show_topk)
                    # print(batch_idx, " accuracy ", accuracy)

                    # Average loss and accuracy across processes for logging
                    if self.arg.distributed:
                        reduced_loss = reduce_tensor(loss.data)
                        for i in range(len(self.arg.show_topk)):
                            accuracy[i] = reduce_tensor(accuracy[i])
                            # prec5 = reduce_tensor(prec5)
                    else:
                        reduced_loss = loss.data
                    ...
                    # print(batch_idx, " accuracy after reduce", accuracy)
            ...
            if accuracy[0] > self.best_acc:
                self.best_acc = accuracy[0]
                self.best_acc_epoch = epoch + 1

            if self.arg.rank == 0:            
                print('Accuracy: ', accuracy[0], ' model: ', self.arg.work_dir)
                if self.arg.phase == 'train' and not self.arg.debug:
                    self.val_writer.add_scalar('loss', reduced_loss, self.global_step)
                    # self.val_writer.add_scalar('loss_l1', l1, self.global_step)
                    self.val_writer.add_scalar('acc', accuracy[0].item(), self.global_step)

                # score_dict = dict(zip(self.data_loader[ln].dataset.sample_name, score))
                self.print_log(f'\tMean {ln} loss of {len(self.data_loader[ln])} batches: {np.mean(loss_values)}.')
                for i, k in enumerate(self.arg.show_topk):
                    self.print_log(f'\tTop {k}: {accuracy[i].item():.2f}%')
